...
import com.example.khind.fragmentActivity1.LoginSuccess
...
class Status : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(ShareReference.getAlarm() == "clear"){
            val fragmentTransaction= fragmentManager?.beginTransaction()
            fragmentTransaction?.add(R.id.frame_layout_status, StatusClear())
            fragmentTransaction?.addToBackStack(null)
            fragmentTransaction?.commit()
        }
        else if(ShareReference.getAlarm() == "alert"){
            val fragmentTransaction= fragmentManager?.beginTransaction()
            fragmentTransaction?.add(R.id.frame_layout_status, StatusAlert())
            fragmentTransaction?.addToBackStack(null)
            fragmentTransaction?.commit()
        }
        else if(ShareReference.getAlarm() == "warning"){
            val fragmentTransaction= fragmentManager?.beginTransaction()
            fragmentTransaction?.add(R.id.frame_layout_status, StatusWarning())
            fragmentTransaction?.addToBackStack(null)
            fragmentTransaction?.commit()
        }
